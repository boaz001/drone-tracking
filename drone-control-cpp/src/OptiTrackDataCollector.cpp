...
COptiTrackDataCollector::connect()
{
  ...
  if (pNatNetClient_ == NULL)
  {
    ...
    if (pNatNetClient_ != NULL)
    {
      ...
      const char szMyIPAddress[128] = "127.0.0.1\0"; // or \n ?
      const char szServerIPAddress[128] = "127.0.0.1\0"; // or \n ?
      ...
      else
      {
        ...
        std::cout << "Application: '" << serverDescription.szHostApp << "' version: " << serverDescription.HostAppVersion[0] << "." serverDescription.HostAppVersion[1] << "." << serverDescription.HostAppVersion[2] << "." << serverDescription.HostAppVersion[3] << std::end;
        std::cout << "NatNet version: " serverDescription.NatNetVersion[0] << "." serverDescription.NatNetVersion[1] << "." << serverDescription.NatNetVersion[2] << "." << serverDescription.NatNetVersion[3] << std::end;
